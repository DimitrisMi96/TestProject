...
namespace automation
{
    ...
    public class TestGmail
    {
        ...
        {
            WebDriverWait wait = new WebDriverWait(driver, TimeSpan.FromSeconds(10));                              //Delay value (wait)
            
            driver.Manage().Window.Maximize();                                                                     //Browser to Full Screen
            driver.Navigate().GoToUrl("https://gmail.com");                                                        //Test gets a URl

            
            driver.FindElement(By.Id("identifierId")).SendKeys("automationt42@gmail.com");                         //Entering Gmail

            
            driver.FindElement(By.XPath("//*[@id='identifierNext']/div/button/span")).Click();                     //Next Button Click

           
            IWebElement password = wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementIsVisible(By.XPath("//*[@id='password']/div[1]/div/div[1]/input")));  //Using our delay value so the textbox can appear
            ...
            IWebElement primaryTab = wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementIsVisible(By.Id(":1w")));   //Delay value (wait) is being used in order for the primary tab to appear after redirecting.
            if (primaryTab.GetAttribute("aria-selected").Equals("false"))
            {
                
                primaryTab.Click();                                                                                //Clicking the Primary tab only if its not Selected
                Console.WriteLine("primary tab selected");
            }
            ...
            driver.FindElement(By.XPath("//*[@id=':2i']")).Click();                                                 //click on Email to Open
